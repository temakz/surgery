...
function build_email_body(string $formType, array $data): string
{
  $lines = [
    'Тип формы: ' . $formType,
    'Дата: ' . date('c'),
...
function send_form_email(array $config, string $formType, array $data, array $files): array
{
  ...
  try {
    ...
    $mail->setFrom((string) ($smtp['from_email'] ?? 'forms@cmf-surgery.ru'), (string) ($smtp['from_name'] ?? 'Center of Surgery'));
    ...
    $mail->Body = build_email_body($formType, $data);
    $mail->isHTML(false);

    foreach ($files as $file) {
      if (isset($file['tmp_name'], $file['name']) && is_uploaded_file((string) $file['tmp_name'])) {
        $mail->addAttachment((string) $file['tmp_name'], (string) $file['name']);
      }
    }
